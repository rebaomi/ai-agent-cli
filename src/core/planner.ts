...
- 像“打开谷歌/百度并搜索关键词”这类单一浏览器任务，不要拆成泛化的多步骤计划；如果必须输出 toolCalls，直接输出一个完整的 browser_automate，并包含 wait_for_selector + fill + press(Enter) + wait_for_selector，不能只给 press 或 extract_text
...
- 如果任务是“延续上一任务”且用户跟进只是在改格式、改标题、改输出路径、补发送飞书/发我飞书，不要重新搜索、重新调研或重做正文分析；默认复用上一任务已有内容或 $LAST_ASSISTANT_TEXT
- 如果任务目标同时包含“整理/导出文档”和“发到飞书”，最终计划里不能只停在导出；必须保留“发送到飞书”这个最终交付目标
...
export class Planner {
  ...
  async createPlan(task: string): Promise<Plan> {
    ...
    try {
      ...
      const currentDateContext = this.buildCurrentDatePlanningContext();
      const response = await this.llm.generate([
        { role: 'system', content: [PLANNER_PROMPT, currentDateContext, proceduralContext, memoryContext].filter(Boolean).join('\n\n') },
  ...
  private normalizePlannedToolCall(
    ...
    if (this.shouldSuppressFollowUpAdjustmentToolCall(task, description, toolCall)) {
      return null;
    }
    ...
    if (/^web_search$/i.test(toolCall.name)) {
      ...
      return {
        ...
        args: {
          ...toolCall.args,
          query: this.normalizeTimeSensitiveWebSearchQuery(query, `${task}\n${description}`),
        },
      };
    }

    const browserSearchRequest = this.extractBrowserSearchRequest(`${task}\n${description}`);
    if (browserSearchRequest && /^(browser_automate|open_browser)$/i.test(toolCall.name)) {
      return this.buildBrowserSearchToolCall(browserSearchRequest.engine, browserSearchRequest.query);
    }

    return toolCall;
  }

  private shouldSuppressFollowUpAdjustmentToolCall(
    task: string,
    description: string,
    toolCall: { name: string; args: Record<string, unknown> },
  ): boolean {
    if (!this.isFollowUpAdjustmentTask(task)) {
      return false;
    }
    ...
    if (/^(glob|search_files)$/i.test(toolCall.name) && !this.hasExplicitSourceReference(task, description, toolCall.args)) {
      return true;
    }

    if (/^(read_file|read_multiple_files)$/i.test(toolCall.name) && !this.hasExplicitSourceReference(task, description, toolCall.args)) {
      return true;
    }
  ...
  private isFollowUpAdjustmentTask(task: string): boolean {
    return /延续上一任务：/i.test(task)
      && /用户跟进：/i.test(task)
      && /(改成|改为|换成|换为|转成|转为|转换成|转换为|另存为|保存成|保存为|导出成|导出为|发我飞书|发到飞书|发给飞书|发送到飞书|发送给飞书|标题|文件名|路径)/i.test(task);
  }

  private hasExplicitSourceReference(task: string, description: string, args: Record<string, unknown>): boolean {
    const candidatePaths = [
      typeof args.path === 'string' ? args.path : '',
      typeof args.source === 'string' ? args.source : '',
      typeof args.file === 'string' ? args.file : '',
      Array.isArray(args.paths) ? args.paths.filter((item): item is string => typeof item === 'string').join('\n') : '',
      `${task}\n${description}`,
    ].filter(Boolean).join('\n');

    return this.extractExplicitPaths(candidatePaths).length > 0;
  }

  private buildCurrentDatePlanningContext(): string {
    const now = new Date();
    ...
    return [
      ...
      `- 当前日期：${year}-${month}-${day}`,
      `- 当前年份：${year}`,
      '- 涉及“最近、近期、今年、本周、本月、近一年”等时效性搜索时，默认基于当前日期，不要凭空写旧年份。',
      '- 只有用户明确指定历史年份时，才保留该历史年份。',
    ].join('\n');
  }

  private normalizeTimeSensitiveWebSearchQuery(query: string, taskContext: string): string {
    if (!query) {
      return query;
    }

    const currentYear = new Date().getFullYear();
    const combinedContext = `${taskContext}\n${query}`;
    const hasTimeSensitiveIntent = /最近|近期|今年|本周|本月|近一年|过去一年|最新|当前|最近有什么|近期有什么/i.test(combinedContext);
    if (!hasTimeSensitiveIntent) {
      return query;
    }

    const explicitHistoricalYear = this.extractExplicitHistoricalYear(taskContext);
    if (explicitHistoricalYear) {
      return query;
    }

    const normalizedQuery = query.replace(/\b(20\d{2})\b/g, (match, yearText: string) => {
  ...
  private extractExplicitHistoricalYear(taskContext: string): number | null {
    const matches = Array.from(taskContext.matchAll(/\b(20\d{2})\b/g));
    if (matches.length === 0) {
      return null;
    }
    ...
    for (const match of matches) {
      ...
      const windowStart = Math.max(0, (match.index || 0) - 8);
      const windowEnd = Math.min(taskContext.length, (match.index || 0) + match[0].length + 8);
      const nearby = taskContext.slice(windowStart, windowEnd);
      if (/指定|历史|当年|那年|回顾|复盘|总结|查|搜索|活动|新闻|资料/i.test(nearby)) {
        return year;
      }
    }
  ...
  private inferToolCallsFromText(task: string, description: string): Array<{ name: string; args: Record<string, unknown> }> {
    ...
    const browserSearchRequest = this.extractBrowserSearchRequest(combined);
    if (browserSearchRequest) {
      return [this.buildBrowserSearchToolCall(browserSearchRequest.engine, browserSearchRequest.query)];
    }
  ...
  private extractBrowserSearchRequest(input: string): { engine: 'google' | 'baidu'; query: string } | null {
    ...
    if (!engine || !/(搜索|查找|查询|输入).*(关键词|关键字)?|关键词|关键字/i.test(input)) {
      return null;
    }
    ...
    const patterns = [
      /(?:输入|搜索|查找|查询)(?:关键词|关键字)?[：: ]*["“”']?([^"“”'，。,\.\n]+)["“”']?/i,
      /(?:关键词|关键字)[：: ]*["“”']?([^"“”'，。,\.\n]+)["“”']?/i,
      /(?:在)?(?:google|谷歌|百度)(?:上)?(?:搜索|查找|查询)[：: ]*["“”']?([^"“”'，。,\.\n]+)["“”']?/i,
    ];

    for (const pattern of patterns) {
      const match = input.match(pattern)?.[1]?.trim();
      if (match) {
        return { engine, query: match };
      }
    }
  ...
  private buildBrowserSearchToolCall(engine: 'google' | 'baidu', query: string): { name: string; args: Record<string, unknown> } {
