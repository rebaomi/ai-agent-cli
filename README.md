...
- 🤖 **多模型支持** - 支持 Ollama、Qwen、DeepSeek、Kimi、GLM、Doubao、MiniMax、GPT、Claude、Gemini，以及 Hybrid 混合路由
- 🌐 **自动操作浏览器** - 内置浏览器直达、确定性自动化与智能浏览器代理重构骨架，后续可按你的真实操作路径持续定制
...
- `workflow`：默认模式，优先按任务编排、checkpoint 恢复、权限与预算策略来执行
- `chat`：仅在你明确想闲聊、问答、解释概念时使用
...
/function switch workflow
```

`/agent-mode` 仍然保留为兼容别名，但新的推荐入口是 `/function`。
...
1. **Direct Action**
  ...
  - 现在还支持一个可配置的“对话模式阈值”：命中阈值时不会静默执行，而会先回一句“我先这么做，做完告诉你结果”再执行

当前 session 里也增加了一层显式任务栈：像“继续”“再来”“按刚才那个方式”这类跟进语句，会优先绑定到上一条活跃任务，而不是只靠消息历史做隐式猜测。
...
浏览器自动操作相关配置也已预留：浏览器智能代理默认优先使用本地 Ollama；如果 Ollama 不可用，再回退到 `defaultProvider`。同时支持一个独立的 `browser-workflows/` 目录，用户可以像写 skill 一样写 Markdown 流程文件，通过 `--workflow` 显式指定，或按站点自动匹配。workflow 现在也支持 phase-aware sections，可以显式声明搜索页、结果页、详情页、表单页各自的步骤和 selector 偏好。`/browser workflow lint` 除了做人类可读的 schema 校验，还支持 `--json` 输出稳定的 machine-readable 结构，便于后续在 CLI 或前端里做高亮、过滤和自动修复建议；browser agent 内部也补了一层页面 phase 模型，会区分搜索输入页、结果页、详情页、表单页，减少站点逻辑继续散落在条件分支里。整体设计见 [docs/browser-agent-redesign-plan.md](docs/browser-agent-redesign-plan.md)。

现在还支持参考 browser-use 的方式，把扩展和脚本注入纳入 browser agent 启动链路：

- `browserAgent.extensionPaths`: 启动时加载一个或多个解压后的 Chromium 扩展目录，例如 Tampermonkey。
- `browserAgent.userDataDir`: 持久化扩展状态、登录态、脚本本地存储。
- `browserAgent.initScriptPaths` / `browserAgent.initScripts`: 在页面导航前通过 Playwright `addInitScript` 注入脚本，适合做全局 DOM patch、hook、埋点。
- `browserAgent.pageScriptPaths` / `browserAgent.pageScripts`: 页面加载完成后执行脚本，适合做页面级数据采集或局部 DOM 改写。
- `browserAgent.userscripts`: 把用户脚本按 `document-start` 或 `document-end` 两种时机注入，便于模拟“油猴脚本 + AI 控制”的组合。

在此基础上，browser agent 现在也支持显式脚本动作：

- `evaluate_script`: 直接执行页面 JavaScript，用于读取运行时状态、触发页面内 helper 或快速探测 DOM。
- `call_userscript_api`: 调用用户脚本暴露到全局的 API，例如 `UserscriptBridge.collect`。
- `toggle_userscript_mode`: 显式开启或关闭用户脚本模式，便于在同一任务里切换“原生页面”和“脚本增强页面”。

同一套脚本环境现在也会被 deterministic `/browser run` 复用。也就是说，如果你已经在配置里声明了 `browserAgent.userDataDir`、`extensionPaths`、`initScriptPaths`、`pageScriptPaths`、`userscripts`，那么 `browser_automate` 和 `/browser run` 默认会继承这些设置，不需要再为确定性动作单独复制一套浏览器脚本配置。只有 `browser`、`headless`、`timeoutMs` 这类命令参数会优先覆盖配置默认值。

`toggle_userscript_mode` 现在还会在同一个标签页的后续导航中保持状态，直到下一次显式切换。也就是说，你可以先关闭脚本增强态，跳转几个页面后再继续用“原生页面”观察；或者先打开脚本增强态，让后续导航继续沿用注入脚本能力。

注意：启用 `extensionPaths` 时，运行时会自动切到 Playwright persistent context，并强制使用 `chromium` 通道以兼容扩展侧载；这和 browser-use 的处理方向是一致的。实际使用 Tampermonkey 时，建议把 `headless` 设为 `false`，并提供稳定的 `userDataDir`。

例如下面这种 deterministic 动作文件，会直接复用上面的 browserAgent 扩展和脚本环境：

```json
[
  { "type": "goto", "url": "https://shop.example.com/orders" },
  { "type": "toggle_userscript_mode", "enabled": true },
  {
    "type": "call_userscript_api",
    "api": "UserscriptBridge.collectOrders",
    "args": [20],
    "expectResult": {
      "type": "array",
      "shape": "{ id: string; status: string; total?: number }",
      "description": "Visible order summaries"
    }
  },
  {
...
```bash
/browser run https://shop.example.com @actions.json --headed
```

workflow 现在也支持站点级脚本绑定。可以在 workflow frontmatter 中声明 `scripts` 或 `scriptBinding`，对应站点命中后会自动把脚本带进当前 browser agent session：

```yaml
---
name: shop-order-review
description: Review order list with userscript helpers
match:
  - shop.example.com
priority: 120
scripts:
  userscriptPaths:
    - ./browser-scripts/shop-helper.user.js
  pageScriptPaths:
    - ./browser-scripts/order-page-hook.js
  userscriptMode: on
  userscriptRunAt: document-end
scriptApis:
  - name: UserscriptBridge.collectOrders
    description: Return visible order cards from the current page
    args:
      - name: limit
        type: number
        required: false
        description: Maximum number of orders to collect
    returns:
      type: array
      shape: '{ id: string; status: string; total?: number }'
      description: Visible order summaries
...
如果你的用户脚本在页面上暴露了全局 API，例如 `window.UserscriptBridge.collectOrders = () => ...`，planner 就可以在需要时规划 `call_userscript_api` 动作，而不是继续硬编码 selector 点击流程。现在 `scriptApis.returns` 也支持结构化声明，适合明确告诉 planner 这个 API 返回数组、对象还是纯文本，以及大致字段 shape。

除了 workflow 里的 `scriptApis.returns`，你也可以在 deterministic 动作文件里直接给 `evaluate_script` 或 `call_userscript_api` 声明 `expectResult`。如果 planner 没显式写出这个字段，`call_userscript_api` 会优先复用 workflow 中对应 API 的返回契约。运行时会把“匹配 / 不匹配”、预期类型、实际类型一并写入 action 输出，方便你判断脚本 API 是否已经偏离约定。

`browserAgent.expectResultMismatchStrategy` 现在支持三档运行时策略：

- `record-only`: 不改变动作是否成功，只把 mismatch 记录进 action 输出或日志，适合先观察脚本 API 稳定性。
- `warn`: 默认值。继续执行，但会在 action summary / validation 中明确标记警告。
- `hard-fail`: 一旦 `expectResult` 与实际返回值不匹配，立即中断当前 browser run / browser agent 执行。
...
默认还会启用一层浏览器安全闸门：一旦检测到支付、提现、转账、下单付款、验证码/密码填写、身份证/银行卡等隐私信息提交，或其他明显违规违法操作，自动化会立即停止，并要求用户本人手动完成。相关开关位于 `browserAgent.safety`。

安全拦截现在带有结构化状态：浏览器自动化会返回 `BROWSER_SAFETY_ABORTED` / `browser_safety_abort`，CLI 和上层 agent 可以区分“执行失败”和“安全中止”。同时支持按全局和域名级别配置 `allowKeywords`、`blockKeywords`、`domainPolicies`，用来减少业务站点上的误报。例如可以允许“review order”这类只读步骤继续通过，但仍然阻断 “pay now” 或 “confirm payment”。
...
| `/browser` | 浏览器打开与自动化命令 |
| `/vision` | 使用 Ollama 多模态模型分析单图、图片目录或多个目标 |
| `/news` | 腾讯新闻快捷命令 |
| `/cron` | 管理定时任务与新闻/天气推送 |
...
| 图片理解 | `/vision analyze <fileOrDir> [moreFilesOrDirs...] [prompt...] [--model <name>] [--limit <n>]` |
...
- 显式命令：适合你明确知道要分析哪些目录或文件
- 普通自然语言：适合直接在对话里说“分析某目录下的图片”

示例：

```bash
/vision analyze ./screenshots
/vision analyze ./screenshots ./captures 请总结这些页面里的异常状态
...
```text
分析 D:/screenshots 目录下的图片
看看 ./captures 和 ./screenshots/error.png 里的报错
识别 ./images/login.png 这张图里有哪些异常提示
```

这两种入口最终都会落到同一套 vision service：支持单图、单目录、多目录、目录加单图混合输入；如果配置了 `ollama.visionModel`，默认会走该模型，否则回退到 `minicpm-v`。
...
| Qwen | qwen3.5:9b, qwen2.5-coder:7b | 常见通过 Ollama 本地运行，也可接入兼容 OpenAI API 的 Qwen 服务 |
...
- 🤖 **Multi-Model Support** - Ollama, Qwen, DeepSeek, Kimi, GLM, Doubao, MiniMax, GPT, Claude, Gemini, and hybrid routing
...
If you also want the CLI to receive images or files from Feishu and store them locally before handing the message to the agent, configure the relay receive directory explicitly:
...
notifications:
  lark:
    relay:
      ...
      receiveDir: C:/Users/your-name/.ai-agent-cli/feishuReceive
```

Incoming Feishu images and files are saved under date-based subfolders such as `C:/Users/your-name/.ai-agent-cli/feishuReceive/2026-04-11/`, and the local file path is appended to the message content that gets forwarded into the CLI agent.
